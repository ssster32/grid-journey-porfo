...
- 生成直後の `GridCell.initial_score` と `GridCell.calculated_score` は、`MapArea.region_feature_level` と同じ値になります。
...
| `region_feature_level` | GridCell の初期評価に使う地域特徴レベル |
...
| `east` | そのマスの東端 |
| `west` | そのマスの西端 |
| `initial_score` | 対象 `MapArea.region_feature_level` と同じ値 |
...
現時点では、将来の自動判定に向けた暫定実装として `MapArea.region_feature_level` を全 GridCell の初期評価に使います。
