...
自動設定でメモグリッドを作成した場合、Overpass API への問い合わせに時間がかかるため、作成直後は `grid_generation_status=pending` になり、GridCell がまだ生成されていない場合があります。

pending 状態のメモグリッドは、staff 権限を持つ管理者が以下の画面から 1 件ずつ処理できます。

```text
/maps/admin/pending-grid-jobs/
```

この画面では、現在の pending 件数、次に処理される MapArea、作成者、マス数などを確認できます。
「pendingを1件処理する」ボタンを押すと、古い pending MapArea を 1 件だけ処理します。

処理後の状態は以下のいずれかになります。

- `completed`: 自動初期スコア設定による GridCell 生成が完了
- `fallback_completed`: Overpass API 取得に失敗したが、標準値で GridCell 生成が完了
- `failed`: GridCell 生成に失敗

この処理は Overpass API へ問い合わせるため、10〜20秒程度かかる場合があります。
短時間に連続して実行しないでください。
一般ユーザーはこの画面を表示・実行できません。
また、`failed` や `fallback_completed` の再処理は、この画面の対象外です。

ローカルや Shell が使える環境では、以下の management command でも同じ pending 処理を実行できます。

```bash
python manage.py process_pending_grid_areas --limit 1
```

Render 無料環境など、Shell や Cron Job の利用が難しい場合は、管理者限定画面を使って必要な時に 1 件ずつ処理します。
この画面は Celery / RQ / Redis などの本格的な非同期処理基盤の代わりに使う、簡易的な運用補助です。
...
Mac から Windows へ作業環境を移す場合、コードは GitHub で引き継ぎ、仮想環境とローカル設定は Windows 側で作り直します。
`.venv` は OS ごとに中身が違うため、Mac の `.venv` フォルダは Windows へコピーしません。

### 1. Mac 側で差分を確認する

まず、Mac 側で未コミットの変更が残っていないか確認します。
...
残したい変更がある場合は commit してから GitHub へ push します。
不要な変更がある場合は、内容を確認してから整理します。

```bash
git push origin main
```

ブランチ名が `main` ではない場合は、現在のブランチ名に置き換えます。

### 2. Windows 側でコードを取得する

Windows でまだ clone していない場合は、GitHub から取得します。

```powershell
git clone https://github.com/ssster32/grid-journey-porfo.git
cd grid-journey-porfo
```

すでに clone 済みの場合は、最新状態を取得します。
...
Windows では、Windows 用の `.venv` を新しく作成します。
...
仮想環境が有効な間は、PowerShell の行頭に `(.venv)` が表示されます。

### 4. 環境変数を設定する

通常のローカル開発だけなら、既定値で起動できます。
外部 DB や本番に近い設定を使う場合は、Windows 側でも環境変数を設定します。
...
`DJANGO_SECRET_KEY` や `DATABASE_URL` などの秘密情報は Git に入れません。
Mac 側に `.env` がある場合も、そのまま GitHub へ push せず、必要な値だけ Windows 側で再設定します。

### 5. DB を準備して起動確認する

ローカル SQLite を使う場合は、Windows 側で migration を実行して DB を作ります。
...
### 6. Windows 移行後に確認すること

必要に応じて、次のコマンドで状態を確認します。
...
管理者用の pending 処理画面を使う場合は、staff ユーザーでログインして以下を確認します。

```text
/maps/admin/pending-grid-jobs/
```

移行時に引き継ぐものは、主に GitHub 上のコード、使用ブランチ名、必要な環境変数、ローカル DB を作り直す手順です。
`.venv`、`db.sqlite3`、`.env` は PC ごとのローカルファイルなので、基本的には Git で共有しません。
