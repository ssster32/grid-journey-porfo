...
curl -i -u testuser:test-password \
  ...
  -d '{"name": "Manual Test Area", "description": "manual curl test", "center_lat": 35.695, "center_lng": 139.795, "grid_size_meters": 500, "region_feature_level": 3, "rows": 6, "cols": 8, "source": "manual"}'
```
...
curl -i \
  ...
  -H "Content-Type: application/json" \
  -X POST http://127.0.0.1:8000/api/maps/areas/ \
  -d '{"name": "Manual Token Area", "description": "manual token test", "center_lat": 35.695, "center_lng": 139.795, "grid_size_meters": 500, "region_feature_level": 3, "rows": 6, "cols": 8, "source": "manual"}'
```
...
1. `メモグリッド作成` フォームで、中心緯度・中心経度・1マスの大きさ・地域特徴レベル・縦横のマス数を確認する。
   例: `center_lat=35.695`, `center_lng=139.795`, `grid_size_meters=500`, `region_feature_level=2`, `rows=6`, `cols=8`
2. 必要に応じて値を調整し、`メモグリッドを作成` を押す。
   `north / south / east / west` は入力せず、サーバー側で自動計算されます。
3. 作成したメモグリッドが一覧に表示され、選択状態になることを確認する。
4. `GridCell` が `rows * cols` 件作成され、作成直後のスコアが地域特徴レベルと同じ値になることを確認する。
   上記の例なら `6 * 8 = 48` 件です。
...
14. Map Preview 上の GridCell をクリックし、Score Map と同じ選択状態に反映されることを確認する。
15. Map Preview 上で Shift キーを押しながらドラッグし、範囲内の GridCell をまとめて選択できることを確認する。
16. Shift を押さずに Map Preview をドラッグし、通常どおり地図を移動できることを確認する。
17. `選択中のマス` パネルで、直近の選択・選択数・選択グリッド一覧が更新されることを確認する。
18. `個別に入力し、まとめて採点` を選び、選択中 GridCell ごとに 1 から 10 の score を入力する。
19. `まとめて採点する` を押し、採点中の表示が出た後、採点後に選択が解除されて `Score Map` と `Map Preview` の色分けが更新されることを確認する。
20. 再度複数のマスを選択し、`選択グリッドを全て同じ値で採点` を選ぶ。
21. 共通 score に 1 から 10 の整数を入力し、`同じ値で採点する` を押して Score Map と Map Preview が更新されることを確認する。
22. `選択をすべて解除` で選択状態をリセットできることを確認する。
23. 作成者ユーザーで、`共有相手 username` に `otheruser` を入力して `共有相手を追加` を押す。
24. `共有相手一覧を取得` を押し、共有相手一覧に `otheruser` が表示されることを確認する。
25. 必要に応じて `otheruser` でログインし直し、共有メモグリッドを閲覧・採点できることを確認する。
26. 作成者ユーザーに戻り、`共有を解除` を押して共有相手一覧から消えることを確認する。
...
Score Map と Map Preview のクリック選択は同じ選択状態を使います。
...
`region_feature_level` は、作成時に全 GridCell の `initial_score` と `calculated_score` に入る地域特徴レベルです。
demo ページでは、中心座標・`grid_size_meters`・`region_feature_level`・`rows`・`cols` からメモグリッドを作成します。
