...
- `/maps/<area_id>/`
  - ログイン必須のメモグリッド詳細画面。
  - GridCell 一覧、Map Preview、選択中マス詳細、採点フォーム、共有相手管理、削除を扱う。
...
- `/maps/`
  ...
  - 削除ボタンには「削除すると元に戻せません。」の補足を付けている。
- `/maps/new/`
...
- GridCell 一覧
  - `GET /api/maps/areas/<area_id>/grids/` からマス一覧を取得して表示する。
  - 手動再取得ボタンで GridCell を再取得できる。
- Leaflet Map Preview
  - MapArea 全体枠を表示する。
  - GridCell 境界を表示する。
  - `calculated_score` に応じて境界色を変える。
  - スコア色分け凡例を表示する。
  - GridCell のスコアラベルを表示する。
  - スコアラベルはズームに応じてサイズ class を切り替える。
  - スコアラベルにもスコア帯ごとの色分けを付けている。
- 選択操作
  - 通常クリックで単体選択できる。
...
- 採点
  - 単体選択時は単体採点フォームを表示する。
  - 複数選択時は同一値一括採点フォームを表示できる。
  - 複数選択時はマスごとの個別入力採点フォームにも切り替えられる。
  - 個別入力採点は、既存の単体採点 API をマスごとに順番に呼ぶ。
...
- 詳細画面では、単体採点、同一値一括採点、複数マス個別採点の処理中に表示する。
