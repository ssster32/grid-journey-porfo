...
.venv/bin/python manage.py test maps.tests.MapAreaListViewTests
.venv/bin/python manage.py test maps.tests.MapDemoViewTests
.venv/bin/python manage.py check
git diff --check -- maps/serializers.py maps/static/maps/demo.js maps/static/maps/demo.css maps/tests.py
```
...
- Score Map のマスをクリックして選択し、採点パネルで評価できるようにする。
- 画面遷移は挟まず、demo ページ内で完結させる。
- 既存の単体採点 API をそのまま使う。

採点パネル案:

1. `state.selectedGridId` を追加する。
2. Score Map の `.score-cell` に `data-grid-id` を付ける。
3. Score Map のマスクリックで選択状態にする。
4. 選択中のマスに枠線や影を付ける。
5. Score Map の下または右側に `選択中 GridCell #xx` の採点パネルを表示する。
6. 採点パネルに score 入力と `採点` ボタンを置く。
7. 採点後に `loadGrids()` で Score Map と一覧を再取得する。
8. 既存の `rateGrid(gridId)` を流用できるようにする。

この方針が、実装難易度と使いやすさのバランスがよい。
画面遷移ありの詳細ページは、将来的な本格 UI で検討すればよい。
